...
if ($conn->query($del1) === TRUE) {
	...
	$_SESSION['update']=$_POST['email'];
	$update1=$_SESSION['update'];
	echo $update1;
	header("location:update1.php?email=$update1");
	}
...
if ($allcontacts->num_rows > 0) {
	...
	?>
							<tr>
								...
								<form action="update1.php?email=<?php echo $email;?>" method="post">
								<input type="hidden" name="deleted" value="<?php echo $email;?>"/>
								<button type="submit" class="btn btn-success" name="update" >Update
								</button></form></td>
